...
$(document).ready(function () {
    ...
    let main_menu = $('.depth1 > li');
    let sub_left = $('.sub-left-list li');
    sub_left.eq(0).addClass('sub-focus')
    let sub_right = $('.sub-right > div');
    let sub_close = $('.sub-menu-close');

    $.each(main_menu, function (_index, item) {
        $(this).click(function (e) {
            e.preventDefault();
            header_sub.stop().slideDown();
            user_bt.hide();

            sub_left.removeClass('sub-focus');
            sub_left.eq(_index).addClass('sub-focus')

            sub_right.hide();
            sub_right.eq(_index).show();
        })
    })

    sub_close.click(function () {
        user_bt.show()
        header_sub.stop().slideUp();
    })
